...
async function initRNN() {
  ...
  try {
    await audioContext.audioWorklet.addModule("./NoiseSuppressionProcessor.js");
...
function startWebRTC() {
      ...
      if (ustr(e.data).type == "system") {
        if (ustr(e.data).message == "SCREEN_SHARE_OPENED") {
          ...
          toggleElementDisplay(
            document.getElementById("overlay-remote-audio-icon")
          );
        }
  ...
  document
    .getElementById("submit-offer-answer")
    .addEventListener("click", function () {
      ...
      if (ustr(message.description).type == "offer" && state === 0) {
        ...
        connection.setRemoteDescription(
          new RTCSessionDescription(ustr(message.description)),
          () => {
            connection
              .createAnswer()
              .then(handleLocalDescription)
              .then(addIce(message.candidate));
  ...
  document
    .getElementById("screen-share-button-2")
...
document.getElementById("video-pause-button").addEventListener("click", () => {
  ...
  localVideo.srcObject.getTracks()[1].enabled =
    !localVideo.srcObject.getTracks()[1].enabled;
});
...
document.getElementById("audio-pause-button").addEventListener("click", () => {
  ...
  localVideo.srcObject.getTracks()[0].enabled =
    !localVideo.srcObject.getTracks()[0].enabled;
});

document
  .getElementById("video-pause-button-2")
  .addEventListener("click", () => {
    toggleMediaOptionButtonColor(
      document.getElementById("video-pause-button-2")
    );
    localVideo.srcObject.getTracks()[1].enabled =
      !localVideo.srcObject.getTracks()[1].enabled;
  });
...
document
  .getElementById("audio-pause-button-2")
  .addEventListener("click", () => {
    toggleMediaOptionButtonColor(
      document.getElementById("audio-pause-button-2")
    );
...
function toggleLocalStreamShare() {
  ...
  else document.getElementById("screen-share-local-box").style.display = "none";
  //localScreenShare.srcObject.getTracks()[0].enabled = !localScreenShare.srcObject.getTracks()[0].enabled;
  console.log(
    "Local screen stream ",
    localScreenShare.srcObject.getTracks()[0].enabled
  );
}
...
function toggleRemoteStreamShare() {
  ...
  console.log(
    ...
    remoteScreenShare.srcObject.getTracks()[0].enabled
  );
}
...
function addMessege(a, messege) {
  ...
  if (a == 1)
    chats.innerHTML =
      chats.innerHTML +
      ` <p class="chats me" id = 'chatid${conversation.length}'>${messege}</p>`;
  else
    chats.innerHTML =
      chats.innerHTML +
      ` <p class="chats sender" id = 'chatid${conversation.length}'>${messege}</p>`;

  updateScroll();
}

function sentOverDataStream(type, message) {
  textChannelStream.send(
    str({
      type: type,
      message: message,
    })
...
function initialCSS() {
  ...
  document.getElementById("video-pause-button").style.backgroundColor =
    buttonOpen;
  document.getElementById("audio-pause-button").style.backgroundColor =
    buttonOpen;
...
function addMeetScreen() {
  ...
  document.getElementById("video-pause-button-2").style.backgroundColor =
    buttonOpen;
  document.getElementById("audio-pause-button-2").style.backgroundColor =
    buttonOpen;
  document.getElementById("screen-share-button-2").style.backgroundColor =
    buttonOpen;
...
function toggleMediaOptionButtonColor(element) {
  ...
  else element.style.backgroundColor = buttonClosed;
}
